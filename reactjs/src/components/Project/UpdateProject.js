...
const UpdateProject = ({project,getProject,updateProject,errors}) => {
    ...
    useEffect(() => {
        ...
        // componentDidMount
          ...
          /// already pullin "id" from params, need request  to be EXACT AS SERVER EXPECTS
    const onSubmit = (e) => {
        e.preventDefault();
  ...
  :
  <input className=" bg-slate-700 appearance-none border-2 focus:border-blue-500 rounded w-full py-2 px-3 text-slate-200 mb-3 leading-tight focus:outline-none focus:shadow-outline" disabled id="projectUUID" type="text" placeholder  = "Between 4-5 charecters"  value = {projectUUID} onChange = {(e) => setUUID(e.target.value)}/>
}
    </div>
